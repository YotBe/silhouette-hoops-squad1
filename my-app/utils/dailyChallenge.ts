...
export function isDailyChallengeCompleted(): boolean {
  return storage.getItem('sg_daily_date') === getDailySeed();
}
...
export function saveDailyResult(result: DailyResult): void {
  storage.setItem('sg_daily_date', result.date);
  storage.setItem('sg_daily_result', JSON.stringify(result));
}

export function getDailyResult(): DailyResult | null {
  try {
    const r = storage.getItem('sg_daily_result');
    if (!r) return null;
