...
function loadQuestStorage(): QuestStorage {
  try {
    const raw = storage.getItem('sg_quest_progress');
    if (!raw) throw new Error();
    const data = JSON.parse(raw) as QuestStorage;
    if (data.date !== getDateSeed()) return { date: getDateSeed(), progress: {}, completed: [] };
    return data;
  } catch {
...
function saveQuestStorage(data: QuestStorage): void {
  storage.setItem('sg_quest_progress', JSON.stringify(data));
}

export function getQuestProgress(): { quests: DailyQuest[]; progress: Record<string, number>; completed: string[] } {
  const s = loadQuestStorage();
  return { quests: getTodayQuests(), progress: s.progress, completed: s.completed };
}
...
export function updateQuestProgress(sessionStats: {
  ...
  for (const quest of quests) {
    if (s.completed.includes(quest.id)) continue;
    let progress = 0;
    switch (quest.type) {
      case 'streak': progress = sessionStats.bestStreak; break;
      case 'no_hints': progress = sessionStats.noHintCorrect; break;
      case 'daily_challenge': progress = sessionStats.dailyCompleted ? 1 : 0; break;
      case 'correct_count': progress = sessionStats.totalCorrect; break;
      case 'mystery_mode': progress = sessionStats.mysteryCompleted ? 1 : 0; break;
    }
    const prev = s.progress[quest.id] || 0;
    s.progress[quest.id] = Math.max(prev, progress);
    if (s.progress[quest.id] >= quest.target) {
      s.completed.push(quest.id);
