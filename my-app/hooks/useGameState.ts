...
import { getDailyPlayers, getDailyChoices, isDailyChallengeCompleted, saveDailyResult, getDailySeed } from '@/utils/dailyChallenge';
...
function getStoredHighScores(): Record<string, number> {
  try {
    return JSON.parse(storage.getItem('sg_highscores') || '{}');
  } catch { return {}; }
}

function getStoredXP(): number {
  try { return parseInt(storage.getItem('sg_xp') || '0', 10); } catch { return 0; }
}

function getStoredScoreHistory(): Array<{ score: number; tier: string; date: string; streak: number }> {
  try { return JSON.parse(storage.getItem('sg_history') || '[]'); } catch { return []; }
}

function getStoredMuted(): boolean {
  try { return storage.getItem('sg_muted') === '1'; } catch { return false; }
}
...
export function useGameState() {
  ...
  const toggleMute = useCallback(() => {
    setIsMuted(prev => {
      const next = !prev;
      setSFXMuted(next);
      storage.setItem('sg_muted', next ? '1' : '0');
      return next;
    });
  }, []);
  ...
  const submitAnswer = useCallback((playerId: string) => {
    ...
    setState(prev => {
      ...
      if (correct) {
        hapticSuccess();
        markCollected(prev.currentPlayer!.id);
        ...
        try {
          const dist = JSON.parse(storage.getItem('sg_guess_distribution') || '{}');
          const key = String(prev.hintsRevealed.length);
          dist[key] = (dist[key] || 0) + 1;
          storage.setItem('sg_guess_distribution', JSON.stringify(dist));
        } catch {}
        try {
          const era = getPlayerEra(prev.currentPlayer!);
          const eraStats = JSON.parse(storage.getItem('sg_era_stats') || '{}');
          if (!eraStats[era]) eraStats[era] = { correct: 0, total: 0 };
          eraStats[era].correct += 1;
          eraStats[era].total += 1;
          storage.setItem('sg_era_stats', JSON.stringify(eraStats));
        } catch {}
      } else {
        hapticError();
        try {
    ...
    setState(prev => {
      if (prev.isDailyMode) {
        ...
        if (nextRound >= prev.dailyPlayers.length) {
          ...
          setXP(newXP);
          storage.setItem('sg_xp', String(newXP));
          ...
          storage.setItem('sg_history', JSON.stringify(newHistory));
      ...
      if (prev.lives <= 0) {
        ...
        if (prev.score > tierHighScore) {
          ...
          setHighScores(newHS);
          storage.setItem('sg_highscores', JSON.stringify(newHS));
        }
        ...
        return { ...prev, phase: 'gameover' as GamePhase, leveledUp: xpToLevel(newXP) > prevLevel, newLevel: xpToLevel(newXP) };
      }
  ...
  // Save scores + achievements on game over
  useEffect(() => {
