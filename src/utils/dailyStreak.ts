...
/** Check if a streak milestone reward should be granted. Returns the reward or null. */
export function claimStreakReward(count: number): StreakReward | null {
  const lastRewarded = parseInt(localStorage.getItem(REWARD_KEY) || '0', 10);
  const reward = STREAK_REWARDS.find(r => r.milestone === count && count > lastRewarded);
  if (!reward) return null;
  awardPowerUp(reward.type);
  localStorage.setItem(REWARD_KEY, String(count));
  return reward;
}
