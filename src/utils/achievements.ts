...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  {
    id: 'correct_10', title: 'Double Digits', description: 'Get 10 correct guesses', icon: '🔟',
    condition: s => s.totalCorrect >= 10,
    progress: s => Math.min(s.totalCorrect / 10, 1),
    progressLabel: s => `${s.totalCorrect}/10`,
  },
  {
    id: 'correct_50', title: 'Half Century', description: 'Get 50 correct guesses', icon: '🏅',
    condition: s => s.totalCorrect >= 50,
    progress: s => Math.min(s.totalCorrect / 50, 1),
    progressLabel: s => `${s.totalCorrect}/50`,
  },
  {
    id: 'correct_100', title: 'Century Club', description: 'Get 100 correct guesses', icon: '💯',
    condition: s => s.totalCorrect >= 100,
    progress: s => Math.min(s.totalCorrect / 100, 1),
    progressLabel: s => `${s.totalCorrect}/100`,
  },
  {
    id: 'streak_3', title: 'Hot Hand', description: 'Get a 3-game streak', icon: '🔥',
    condition: s => s.bestStreak >= 3,
    progress: s => Math.min(s.bestStreak / 3, 1),
    progressLabel: s => `${s.bestStreak}/3`,
  },
  {
    id: 'streak_5', title: 'On Fire', description: 'Get a 5-game streak', icon: '🔥',
    condition: s => s.bestStreak >= 5,
    progress: s => Math.min(s.bestStreak / 5, 1),
    progressLabel: s => `${s.bestStreak}/5`,
  },
  {
    id: 'streak_10', title: 'Unstoppable', description: 'Get a 10-game streak', icon: '⚡',
    condition: s => s.bestStreak >= 10,
    progress: s => Math.min(s.bestStreak / 10, 1),
    progressLabel: s => `${s.bestStreak}/10`,
  },
  {
    id: 'xp_500', title: 'Rising Star', description: 'Earn 500 XP', icon: '⭐',
    condition: s => s.totalXP >= 500,
    progress: s => Math.min(s.totalXP / 500, 1),
    progressLabel: s => `${s.totalXP}/500 XP`,
  },
  {
    id: 'xp_2000', title: 'All-Star', description: 'Earn 2000 XP', icon: '🌟',
    condition: s => s.totalXP >= 2000,
    progress: s => Math.min(s.totalXP / 2000, 1),
    progressLabel: s => `${s.totalXP}/2000 XP`,
  },
  {
    id: 'collect_10', title: 'Card Collector', description: 'Collect 10 player cards', icon: '🃏',
    condition: s => s.collectedPlayers >= 10,
    progress: s => Math.min(s.collectedPlayers / 10, 1),
    progressLabel: s => `${s.collectedPlayers}/10`,
  },
  {
    id: 'collect_50', title: 'Gallery Master', description: 'Collect 50 player cards', icon: '🖼️',
    condition: s => s.collectedPlayers >= 50,
    progress: s => Math.min(s.collectedPlayers / 50, 1),
    progressLabel: s => `${s.collectedPlayers}/50`,
  },
...
export function updateAchievementStats(partial: Partial<AchievementStats>): void {
  ...
  // For bestStreak, always keep the max
